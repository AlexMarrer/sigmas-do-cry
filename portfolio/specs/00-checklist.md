...
- [x] Place filter → [photos.md](photos.md) § Filtering & scale — DONE (2026-08-09), pulled forward at 3 trips / 73 photos: chip rows over `country` **and** `year`, `?place=&year=` mirrored via `Location.replaceState` (a router navigation fires a view transition per click), read back once after hydration (⚠ A5). Same pass capped each masonry at 9 tiles behind "Show all N photos" and gave the tiles the scroll reveal on a wrapper cell — details + landmines in [06-gallery.md](06-gallery.md).
- [ ] Split to `/gallery` + `/gallery/[slug]` → [photos.md](photos.md) § Filtering & scale — trigger: ~6–8 trips / ~120 photos, or when the chip row outgrows its line. Index becomes one cover card per trip (`Trip.cover`), slugs enumerated for prerender like the project ones (⚠ C1).
